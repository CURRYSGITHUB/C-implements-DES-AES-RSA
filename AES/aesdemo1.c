...
void AES(const byte* inputtext, const byte* inputkey,const uint8_t keylength, const char mode, byte* output) {
	/*接收和输出的都是字节序列 （十六进制数值） 
	AES 加解密函数，
	@inputtext: 输入的字节序列指针，大小为128位即16字节
	@inputkey: 输入的密钥字节序列指针，大小由keylength指定
	@keylength: 输入的密钥长度，只能为128，192，256三个值(位)
	@mode: 模式，'e'为加密模式，'d'为解密模式
	@output: 输出的字节序列指针，大小为128位即16字节
	*/
	int i,j,k = 0;
	int u,v,w=0;
	int count = 0;
...
void CFBe(const byte* plaintext, const uint64_t plainlength, const byte* keytext, const uint8_t keylength, const byte* vitext, byte** ciphertext, uint64_t cipherlength) {
	...
	*/
	...
	int u,v,w;
	//将明文分组 
	MStringToHex(plaintext,DM,plainlength);
	DM[plainlength]='\0';
	//初始向量放入寄存器 
	byte rig[17];
...
int main() {
	...
	-c cipherfile 指定密文文件的位置和名称。
	*/
	plainfile = "plaintest.txt"; 
	keyfile = "aes_key.txt";
	vifile = "aes_iv.txt";
	mode = "OFB";
	cipherfile = "aes_Cipher.txt";
